...
class TestGlobalRegistration:
    ...
    def test_get_logify_logger_configures_once(self, temp_log_dir):
        ...
        log2 = get_logify_logger(
            "test_config_once",
            log_dir=temp_log_dir,
            log_file="different.log"  # Different file
        )

        # Should still have same handlers (not reconfigured)
        assert len(log2.handlers) == handler_count

    def test_get_logify_logger_no_kwargs(self, temp_log_dir):
        """Zero-config still configures the logger with default handlers."""
        setup_logify()

        log = get_logify_logger("test_no_kwargs", log_dir=temp_log_dir)

        # Zero-config usage must still produce working handlers.
        assert len(log.handlers) > 0
        assert log.config["output"] == "both"
