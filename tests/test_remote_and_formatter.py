...
class TestRemoteHandler:
    ...
    def test_successful_emit(self, mock_post):
        ...
        assert call_args.kwargs["json"]["service"] == "test"
...
class TestFormatter:
    ...
    def test_default_formatter(self):
        """Plain text formatter renders timestamp, level, location, message."""
        formatter = get_formatter(json_mode=False, color=False)

        assert isinstance(formatter, logging.Formatter)
        assert formatter.__class__.__name__ == "PlainLogifyxFormatter"

        record = logging.LogRecord(
            name="svc", level=logging.WARNING, pathname="x.py",
            lineno=7, msg="hello", args=(), exc_info=None, func="run",
        )
        line = formatter.format(record)

        assert "WARNING" in line
        assert "svc:run:7" in line
        assert "hello" in line
        assert "\033[" not in line, "plain formatter must not emit color codes"

    def test_json_formatter(self):
        """Test JSON formatter."""
        formatter = get_formatter(json_mode=True, color=False)

        assert formatter.__class__.__name__ == "CompactJsonFormatter"

    def test_color_formatter(self):
        """Test colored formatter."""
        formatter = get_formatter(json_mode=False, color=True)

        assert formatter.__class__.__name__ == "LogifyxFormatter"
